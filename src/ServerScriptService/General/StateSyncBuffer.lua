...
function StateSyncBuffer.createNoOp()
	local noop = {}
	setmetatable(noop, StateSyncBuffer)

	function noop:add(update: StateUpdate.type) end
	function noop:addStep(await: boolean, updates: table) end
	function noop:addAwaitingStep() end
	function noop:flush() end
