...
function Factory:createInstance(mapNodeType, dependencies)
	...
	elseif mapNodeType == MapNodeTypes.EVENT then
		local eventId = EventSelector.getEventId(dependencies)
		--TODO: implement if no eventId is present
		if Probability.roll(Constants.MAP_SETTINGS.EVENT_CHANCES.SHOP) then
			dependencies.mapNodeType = MapNodeTypes.SHOP
			return ShopInstance.new(dependencies)
		elseif Probability.roll(Constants.MAP_SETTINGS.EVENT_CHANCES.MONSTER) or not eventId then
			dependencies.mapNodeType = MapNodeTypes.REGULAR_ENEMY
			dependencies.stageData = require(game:GetService("ReplicatedStorage").Stages.Level1).tier1[1] --TODO: change this to be dynamic
			return GameInstance.new(dependencies)
		end
		dependencies.playerState:visitEvent(eventId)
		local event = BaseEvent.new(eventId)
