...
function ChestInstance.new(dependencies)
	...
	local executeCommand = function(gameEventType, ...)
		self:fireGameEvent(gameEventType, ...)
	end
	self.rewardsHandler:initRewards(dependencies.playerState, dependencies.echoManager, dependencies.stageData.rewards, executeCommand, self.idGenerator)
